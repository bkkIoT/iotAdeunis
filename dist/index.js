    ...
    var Decoder = /** @class */ (function () {
        ...
        Decoder.prototype.getActiveParsers = function (deviceType, frameCode) {
            ...
            if (deviceType) {
                ...
                var selectedParsers = this.parsers.filter(function (p) { return p.deviceType === deviceType &&
                    (p.frameCode < 0 || p.frameCode === frameCode); });
                activeParsers = activeParsers.concat(selectedParsers);
            }
            else {
                // Get parsers for any device types or any frame codes
                activeParsers = this.parsers.filter(function (p) { return p.deviceType === 'any' &&
                    (p.frameCode < 0 || p.frameCode === frameCode); });
    ...
    var Encoder = /** @class */ (function () {
        function Encoder() {
            ...
            this.builders = [
                ...
                new codec.Comfort0x10Builder(),
                new codec.Motion0x10Builder(),
...
(function (codec) {
     ...
     */
    var Comfort0x10InputData = /** @class */ (function () {
        function Comfort0x10InputData() {
            this.readingFrequency = 600;
        }
        return Comfort0x10InputData;
    }());
    codec.Comfort0x10InputData = Comfort0x10InputData;
    /**
     * Comfort 0x10 (configuration) frame builder
     */
    var Comfort0x10Builder = /** @class */ (function () {
        function Comfort0x10Builder() {
            this.deviceType = 'comfort';
            this.frameCode = 0x10;
            this.inputDataClass = Comfort0x10InputData;
        }
        Comfort0x10Builder.prototype.buildFrame = function (inputData, network) {
         ...
         */
        Comfort0x10Builder.prototype.sanitizeUInt16 = function (unsafeUInt16) {
            return Math.max(0, Math.min(Math.trunc(unsafeUInt16), Math.pow(2, 16) - 1));
        };
        return Comfort0x10Builder;
    }());
    codec.Comfort0x10Builder = Comfort0x10Builder;
})(codec || (codec = {}));
var codec;
(function (codec) {
    /**
...
(function (codec) {
    ...
    var Generic0x20Parser = /** @class */ (function () {
        ...
        Generic0x20Parser.prototype.parseFrame = function (payload, configuration, network) {
            var appContent = { type: '0x20 Configuration' };
            // Content depends on network
            switch (network) {
                case 'lora868':
                    appContent = __assign({}, appContent, this.parseLora868(payload));
                    break;
                case 'sigfox':
                    appContent = __assign({}, appContent, this.parseSigfox(payload));
                    break;
                // case 'unknown':
                default:
         ...
         * @param payload payload
         */
        Generic0x20Parser.prototype.parseLora868 = function (payload) {
            var appContent = {};
         ...
         * @param payload payload
         */
        Generic0x20Parser.prototype.parseSigfox = function (payload) {
            var appContent = {};
...
(function (codec) {
    ...
    var GenericStatusByteParser = /** @class */ (function () {
        ...
        GenericStatusByteParser.prototype.parseFrame = function (payload, configuration, network) {
            var statusContent = {};
...
(function (codec) {
     ...
     */
    var Motion0x10InputData = /** @class */ (function () {
        function Motion0x10InputData() {
            this.readingFrequency = 600;
        }
        return Motion0x10InputData;
    }());
    codec.Motion0x10InputData = Motion0x10InputData;
    /**
     * Motion 0x10 (configuration) frame builder
     */
    var Motion0x10Builder = /** @class */ (function () {
        function Motion0x10Builder() {
            this.deviceType = 'motion';
            this.frameCode = 0x10;
            this.inputDataClass = Motion0x10InputData;
        }
        Motion0x10Builder.prototype.buildFrame = function (inputData, network) {
            var payload = Buffer.alloc(10);
            payload[0] = this.frameCode;
            // reading frequency = acquisition period * historization period
            var dataAcquisitionPeriod = inputData.readingFrequency / number_of_sampling_before_historization_DEFAULT;
         ...
         * @param unsafeUInt16 unsafe UInt16
         */
        Motion0x10Builder.prototype.sanitizeUInt16 = function (unsafeUInt16) {
            return Math.max(0, Math.min(Math.trunc(unsafeUInt16), Math.pow(2, 16) - 1));
        };
        return Motion0x10Builder;
    }());
    codec.Motion0x10Builder = Motion0x10Builder;
})(codec || (codec = {}));
var codec;
(function (codec) {
    /**
...
var AdeunisCodecsCli = /** @class */ (function () {
    ...
    AdeunisCodecsCli.prototype.helpDecode = function () {
        console.log(chalk_1.default.bold('Usage:') + " codec " + chalk_1.default.blue('decode') + " <frame1> [<frame2> ...]"
            + " [--deviceType <device_type>] [--network lora868|sigfox]\n");
        console.log('Available frame codes and device types:');
        var table = new cli_table_1.default({ head: ['Frame code', 'Device type'] });
        var rows = this.decoder.getSupported()
            .filter(function (sf) { return sf.frameCode !== -1; })
            .sort(function (sfa, sfb) {
            var diff = sfa.frameCode - sfb.frameCode;
            if (diff === 0) {
                diff = sfa.deviceType.localeCompare(sfb.deviceType);
            }
            return diff;
        })
            .map(function (sf) { return (['0x' + sf.frameCode.toString(16), sf.deviceType]); });
        ...
        for (var _i = 0, frames_2 = frames; _i < frames_2.length; _i++) {
            ...
            for (var key in content) {
                if (content.hasOwnProperty(key)) {
                    var row = {};
                    row[key] = this.defaultWhenNil(content[key], '');
                    table.push(row);
                }
            }
            console.log(table.toString());
